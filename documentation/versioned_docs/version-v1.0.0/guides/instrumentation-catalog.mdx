...
1. **Bundled baseline.** Compiled into the `agent-manager-service` binary from `.github/release-config.json`. This is what every install gets out of the box with no extra configuration. The list is also visible in the [version mapping table](./amp-instrumentation.mdx#amp-instrumentation-version-mapping).
...
Walkthrough for adding a newly-published `amp-instrumentation` version to a running install. Examples use `0.5.0` as the placeholder — substitute the real version you're adding.
...
To make `0.5.0` the version pre-selected on the create-agent form, set `defaultInstrumentationVersion` in the same `values.yaml`:

```yaml
agentManagerService:
  config:
    otel:
      defaultInstrumentationVersion: "0.5.0"
      additionalInstrumentationVersions:
...
|---|---|---|
| `extension entry "0.5.0": missing imageRepository` | A required field is missing on the entry | Add the field in step 3 |
| `default instrumentation version "0.5.0" not in effective set` | Step 4 set the default but step 3 didn't add the entry | Add the entry, or revert the default |
...
Open the Console, navigate to the create-agent form, and hard-refresh (`Cmd+Shift+R` / `Ctrl+Shift+R`) to bypass the in-browser query cache. The "AMP Instrumentation Version" dropdown should now offer `0.5.0`. If you did step 4, it should also be pre-selected.

### 8. Smoke-test with a real agent

The catalog check verifies that `0.5.0` is *selectable*. It does not verify that the init-container image is *pullable*. Catch any registry-side issue now, not later:

1. Create a new agent → Python language → pick a Python version → pick `0.5.0`.
2. Deploy.
...
- The specific Python tag wasn't mirrored (e.g. you mirrored `0.5.0-python3.11` but the user picked 3.10).
...
`OTEL_DEFAULT_INSTRUMENTATION_VERSION` (chart value: `agentManagerService.config.otel.defaultInstrumentationVersion`) is the version pre-selected on the create-agent form when a user doesn't override it. It defaults to `0.4.1`.
