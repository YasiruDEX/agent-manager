...
3. Click **Deploy**.
4. On the relevant environment card, click **Configure** next to **Configurations and Secrets**.
5. In the configuration drawer, under **Endpoint Authentication**, verify that **Enable API key security** is turned on.
6. Deploy the agent.
...
- API key security and [JWT authentication](./secure-agents-with-jwt-authentication.mdx) are mutually exclusive — an agent uses one or the other, not both.
