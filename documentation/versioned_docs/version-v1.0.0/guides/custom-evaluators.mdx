...
This guide walks you through creating custom evaluators in the AMP Console. Custom evaluators let you define domain-specific quality checks using Python code or LLM judge prompt templates.

## Prerequisites

- A running AMP instance (see [Quick Start](../get-started/quick-start.mdx))
...
:::info Disallowed imports
Source containing `os`, `subprocess`, `socket`, `ctypes`, or `importlib` imports (or a dynamic `__import__()` call) is rejected at creation/update time with a 400 error naming the pattern. This check always applies regardless of deployment, but it's a basic content check, not a full sandbox — write evaluators that don't need these regardless, since network egress is only restricted on deployments where it's enabled and enforced (see [Network access](../concepts/evaluation.mdx#custom-evaluators)).
:::
