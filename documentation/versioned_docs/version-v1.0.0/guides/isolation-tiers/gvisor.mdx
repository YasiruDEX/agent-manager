...
```bash
curl -fsSL https://raw.githubusercontent.com/wso2/agent-manager/amp/v1.0.0/deployments/setup/install-gvisor.sh \
  | sudo bash
```
...
If your cluster was created by the [installation guides](../on-k3d.mdx) (cluster name `amp-local`) rather than the dev Makefile, point the script at it:
