...
Choose `--cpu` and `--memory` now rather than adjusting them later. Changing either requires restarting the VM, which can leave an already-created k3d cluster unable to start. [Troubleshooting](#troubleshooting) covers the recovery if you hit this.
...
:::warning On Colima, resizing is not a free action
Changing `--cpu` or `--memory` requires stopping and restarting the VM, which rebuilds Docker's networks underneath the cluster. An already-created k3d cluster may not come back. Read the next entry before you resize.
:::

</details>

<details>
<summary>k3d server container crash-loops after restarting or resizing Colima</summary>

This does not present as a Kubernetes error. `kubectl` stops responding altogether, because the API server never starts:

```bash
kubectl cluster-info --context k3d-amp-local
# Hangs, or reports the connection was refused

docker ps -a --filter name=k3d-amp-local
# k3d-amp-local-server-0 shows STATUS "Restarting", or repeated "Exited"

docker logs k3d-amp-local-server-0
```

The server node's log ends with:
...
Stopping and starting the Colima VM recreates the `k3d-amp-local` bridge network, and the containers rejoin it in a different order than before. The load balancer can take the address the server node previously held. k3s still has the old address in its persisted node record, so its network policy controller finds no interface matching it and the process exits. Restarting again does not help, because the mismatch is stored in the node's own state rather than recalculated at boot.

Recreate the cluster. Nothing in this install is persistent across a cluster delete, so recovery is the same work as a fresh install:

```bash
k3d cluster delete amp-local
docker network rm k3d-amp-local 2>/dev/null || true

colima stop --profile agent-manager
colima start --profile agent-manager \
  --vm-type=vz --vz-rosetta --network-address \
  --cpu <cpus> --memory <gb>
```

Then rerun [Step 1: Create k3d Cluster](#step-1-create-k3d-cluster) onward.
...
| File | Purpose |
|------|---------|
| [k3d-config.yaml](https://raw.githubusercontent.com/wso2/agent-manager/amp/v1.0.0/deployments/quick-start/k3d-config.yaml) | k3d cluster with all required port mappings |
| [coredns-amp-custom.yaml](https://raw.githubusercontent.com/wso2/agent-manager/amp/v1.0.0/deployments/k8s/coredns-amp-custom.yaml) | CoreDNS rewrites for `*.openchoreo.localhost`, `*.amp.localhost`, `*.agentmanager.localhost`, `*.am-gateway.localhost`, `*.gateway.localhost` |
| [add-environment-thunder.sh](https://raw.githubusercontent.com/wso2/agent-manager/amp/v1.0.0/deployments/scripts/add-environment-thunder.sh) | Provisions the per-environment Thunder identity provider |
| [values-cp.yaml](https://raw.githubusercontent.com/wso2/agent-manager/amp/v1.0.0/deployments/single-cluster/values-cp.yaml) | Control Plane — Backstage disabled, AMP Thunder OIDC |
| [values-dp.yaml](https://raw.githubusercontent.com/wso2/agent-manager/amp/v1.0.0/deployments/single-cluster/values-dp.yaml) | Data Plane — gateway ports, Fluent Bit config |
| [values-op.yaml](https://raw.githubusercontent.com/wso2/agent-manager/amp/v1.0.0/deployments/single-cluster/values-op.yaml) | Observability Plane — standalone OpenSearch, AMP Thunder OIDC |
| [values-openbao.yaml](https://raw.githubusercontent.com/wso2/agent-manager/amp/v1.0.0/deployments/single-cluster/values-openbao.yaml) | OpenBao — dev mode, Kubernetes auth, pre-seeded secrets |
| [oc-collector-configmap.yaml](https://raw.githubusercontent.com/wso2/agent-manager/amp/v1.0.0/deployments/values/oc-collector-configmap.yaml) | Custom OTel Collector ConfigMap for trace ingestion |
| [wso2-amp-api-platform-gateway-extension](https://github.com/wso2/agent-manager/tree/amp/v1.0.0/deployments/helm-charts/wso2-amp-api-platform-gateway-extension) | API Platform Gateway Helm chart (includes gateway config, bootstrap, CR) |
| [otel-collector-rest-api.yaml](https://raw.githubusercontent.com/wso2/agent-manager/amp/v1.0.0/deployments/values/otel-collector-rest-api.yaml) | OTel Collector REST API resource |
