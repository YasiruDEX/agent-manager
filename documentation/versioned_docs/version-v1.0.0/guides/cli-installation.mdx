...
:::info Prerequisites
You need a reachable Agent Manager instance before logging in. Follow the [Quick Start](../get-started/quick-start.mdx) or [On k3d](./on-k3d.mdx) guide first if you have not installed one. The local install exposes the Agent Manager API at `http://api.amp.localhost:8080`.
:::
...
```bash
curl -fsSL https://wso2.github.io/agent-manager/install.sh | AMCTL_VERSION=1.0.0 AMCTL_INSTALL_DIR="$HOME/bin" sh
```
:::

### Windows (x64)

Download the release archive from the [Agent Manager releases page](https://github.com/wso2/agent-manager/releases). The commands below use **`1.0.0`** as an example — replace it with the latest version.

```powershell
Invoke-WebRequest -Uri https://github.com/wso2/agent-manager/releases/download/amp/v1.0.0/amctl_v1.0.0_windows_amd64.zip -OutFile amctl.zip
