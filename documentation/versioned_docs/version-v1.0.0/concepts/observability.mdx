...
The contract is layered on the OpenTelemetry GenAI semantic conventions (`gen_ai.*` + `db.*`), with a few `traceloop.*` extension keys for the gaps OTel hasn't standardized yet. See the [manual instrumentation reference](../guides/amp-instrumentation.mdx#manual-instrumentation) on the `amp-instrumentation` page for the full attribute table, transport details, and a worked `init_otel()` example.

---

## Trace Sampling

By default every trace an agent produces is exported. At high request rates that becomes expensive, so the OpenTelemetry SDK can be told to keep only a fraction of traces. Sampling is a head-based decision made inside the agent process, configured with the standard `OTEL_TRACES_SAMPLER` and `OTEL_TRACES_SAMPLER_ARG` environment variables — see [Configure Trace Sampling](../guides/configure-trace-sampling.mdx).
