...
- An active [AI Gateway](../concepts/gateway.mdx) for the environments you want to configure. On Agent Manager Cloud these are provisioned for you.
...
1. Log in to the WSO2 Agent Manager Console at [console.agent-manager.cloud.wso2.com](https://console.agent-manager.cloud.wso2.com/).
...
Configuration is organized into the tabs below. When you change a setting, an **unsaved changes** banner appears — click **Save** to persist or **Cancel**/**Discard** to revert. Every tab except **Overview** edits the endpoint serving the selected environment — **Manage Tools** additionally shows the endpoint's discovered capabilities alongside its editable access-control settings.
...
| **Auth Type** | The inbound authentication clients must present — `API Key`, `OAuth`, or `None` (configured on the **Security** tab) |
...
Shows everything the selected environment's endpoint exposes, and controls which of it agents can actually reach:

- **Discovered capabilities** — grouped into **Tools**, **Resources**, and **Prompts**, each with a total count. Expand a section to browse individual items and their descriptions. These are captured when you fetch server info for the endpoint.
...
Click **Save** to apply the access-control mode. This is backed by the managed `mcp-acl-list` policy.
...
| Field | Description | Example |
|---|---|---|
| **Method** | `None`, `API Key` (default), or `OAuth` | `API Key` |
| **Key Location** | *(when `API Key` is selected)* Where the key is passed — `header` or `query` | `header` |
| **Header Key** / **Query Param Key** | *(when `API Key` is selected)* The header or query parameter carrying the API key | `X-API-Key` |

Switching methods on a proxy already in use prompts a confirmation before saving, since it can break any agent still configured for the old method until its tool configuration is updated to match.

Click **Save** to persist.

#### OAuth: Scoping Access Per Tool

Selecting **OAuth** reveals an **Authorization** section beneath the method picker, for restricting individual tools to callers whose token carries the right scope:

- **Scopes** sub-tab — the catalog of scopes defined for this proxy. Click **Create Scope** to add one (a name and description); each is deletable from this list. Scopes are defined once for the whole proxy, not per endpoint or environment.
- **Tools** sub-tab — one row per discovered tool, with a multi-select to assign it zero or more scopes from that catalog. A caller needs every scope a tool requires in order to invoke it. A tool already blocked on the **Manage Tools** tab shows a warning icon here, since scoping it is redundant while it's blocked outright.

Full detail on defining scopes and binding them to agent identity roles is in
