...
Creating an agent from a published kind is an alternative to pointing at source code directly (see [Create the Agent](../tutorials/create-your-first-agent.mdx#step-1-create-the-agent) in Create Your First Agent). Either path produces the same kind of running [agent](./agent-lifecycle.mdx); a kind just captures "build once, deploy many times" so a proven agent doesn't need to be rebuilt from scratch for every reuse.
