  ...
  if (schema.type === "object" && schema.properties) {
    ...
    Object.entries(schema.properties).forEach(([key, propSchema]) => {
      ...
      if (
        ...
        (required.includes(key) || hasMeaningfulValue(value))
      ) {
    ...
    if (
      branches.length > 0 &&
      !satisfiesBranchConstraint(branches, values, parentPath)
    ) {
...
const PolicyParameterEditor: React.FC<PolicyParameterEditorProps> = ({
  ...
  const handleAddArrayItem = useCallback(
    (arrayPath: string, itemSchema: ParameterSchema) => {
      setValues((prev) => {
        const current = (getValueByPath(prev, arrayPath) as unknown[]) || [];
